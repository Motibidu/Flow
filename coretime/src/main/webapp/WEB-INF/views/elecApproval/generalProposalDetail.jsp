...
<head>
    ...
    <style>
        ...
        /* [일반 품의서 전용 스타일 - 본문] */
        .content-view { width: 100%; min-height: 400px; padding: 20px; border: 1px solid #ddd; border-radius: 4px; box-sizing: border-box; background-color: #fff; white-space: pre-wrap; line-height: 1.6; font-size: 14px; }
...
<body>
    ...
    <script>
        ...
        function recallDocument(docId) { if(confirm("상신 취소하시겠습니까?")) axios.post("/elecApproval/recall/" + docId).then(res => { alert(res.data.message); location.href = '/elecApproval'; }); }
        function redraftDocument(docId) { location.href = "/elecApproval/redraft/" + docId; }
        function deleteDocument(docId) { if(confirm("영구 삭제하시겠습니까?")) axios.delete("/elecApproval/delete/" + docId).then(res => { alert(res.data.message); location.href = '/elecApproval'; }); }
    </script>
</body>
</html>